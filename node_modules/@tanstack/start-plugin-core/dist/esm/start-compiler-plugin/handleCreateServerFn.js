...
//#region src/start-compiler-plugin/handleCreateServerFn.ts
...
var ssrRpcImporterTemplate = babel.template.expression(`createSsrRpc(%%functionId%%, () => import(%%extractedFilename%%).then(m => m[%%functionName%%]))`);
...
function generateCallerRpcStub(functionId, functionName, extractedFilename, isClientReferenced, envConfig) {
	...
	if (isClientReferenced || !envConfig.ssrIsProvider) return ssrRpcManifestTemplate({ functionId: functionIdLiteral });
...
function handleCreateServerFn(candidates, context) {
	...
	for (const candidate of candidates) {
		...
		if (isProviderFile) {
			...
			const rpcStub = generateCallerRpcStub(functionId, functionName, canonicalExtractedFilename, isClientReferenced, envConfig);
